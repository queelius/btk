"""Soft-delete helpers for bookmark-memex records.
...
def archive(session: Session, instance) -> None:
    """Mark a row as archived.

    Idempotent: re-archiving preserves the original timestamp.
    Caller must commit.
    """
...
def is_archived(instance) -> bool:
    """Return ``True`` if the row currently carries a non-null ``archived_at``."""
